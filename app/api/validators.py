...
CLOSE_PROJECTS_ERROR = 'Закрытые проекты отсутствуют.'
...
async def check_charity_project_exists(
    ...
    Для обновления нужно передать `obj_in: CharityProjectUpdate`:
...
async def check_close_projects(projects: List[dict]):
    """
    Проверяет, есть ли закрытые проекты.
    """
    if not projects:
        raise HTTPException(status_code=404, detail=CLOSE_PROJECTS_ERROR)
